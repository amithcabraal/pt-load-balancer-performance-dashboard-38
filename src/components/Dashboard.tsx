    ...
    <div className="w-full space-y-8 p-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Filter by Endpoint</h2>
        </div>
        <select
            ...
            .map((endpoint) => {
              ...
              return (
                <option 
                  key={endpoint} 
                  value={endpoint}
                  className={endpointInfo?.hasSlowRequests ? 'text-red-600 dark:text-red-400 font-semibold' : ''}
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
          <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Status Code Distribution</h2>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
          <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Request Verb Distribution</h2>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={verbData}
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Processing Time Distribution by Status Code</h2>
              ...
              content={({ payload, label }) => {
                ...
                return (
                  <div className="bg-white dark:bg-gray-800 p-2 border rounded shadow">
                    <p className="font-semibold text-gray-900 dark:text-white">{label} seconds</p>
                    {payload.map((entry: any, index) => (
                      <p key={index} style={{ color: entry.color }}>
                        Status {entry.name}: {entry.value.toLocaleString()} requests
                      </p>
                    ))}
                    <p className="border-t mt-1 pt-1 text-gray-900 dark:text-white">
